...
// GET /api/cluster/incidents
clusterRouter.get('/incidents', async (req, res) => {
  try {
    const data = await getIncidentAuditData();
    res.json(data);
  } catch (error: any) {
    res.status(500).json({ error: error.message || 'Failed to fetch incident audit records' });
  }
});
